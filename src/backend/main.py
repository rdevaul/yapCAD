...
# =============================================================================
# Local Package Loading (for development)
# =============================================================================
...
@app.get("/api/local-package")
async def load_local_package(path: str) -> Response:
    """
    Load a local .ycpkg package and serve as ZIP.
    
    If path points to a directory, zip it on-the-fly.
    If path points to a ZIP file, serve it directly.
    
    This is for development only - allows loading packages from disk.
    """
    pkg_path = Path(path).expanduser().resolve()
    
    if not pkg_path.exists():
        raise HTTPException(status_code=404, detail=f"Package not found: {path}")
    
    # Security check: only allow .ycpkg files/directories
    if not pkg_path.name.endswith('.ycpkg'):
        raise HTTPException(
            status_code=400, 
            detail="Path must end with .ycpkg"
        )
    
    if pkg_path.is_dir():
        # Directory package - zip on the fly
        try:
            content = zip_directory_to_bytes(pkg_path)
            return Response(
                content=content,
                media_type="application/zip",
                headers={
                    "Content-Disposition": f'attachment; filename="{pkg_path.name}"'
                }
            )
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error zipping package: {e}")
    
    elif pkg_path.is_file():
        # Already a ZIP file - serve directly
        return FileResponse(
            pkg_path, 
            media_type="application/zip",
