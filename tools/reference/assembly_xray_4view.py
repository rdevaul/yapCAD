#!/usr/bin/env python3
"""
4-View Matplotlib X-Ray Visualization of Robot Assembly

Generates a 2x2 grid showing:
- Perspective (isometric 3/4 view)
- Front (looking along +X)
- Top (looking along +Z, down)
- Right (looking along -Y)

Uses transparent/wireframe rendering for x-ray effect.
Loads all STL files from kinematic chain and applies world transforms.
No annotation labels for cleaner visualization.
"""
...
def find_stl_file(part_name: str, stl_base_dir: Path) -> Optional[Path]:
    ...
    stl_mappings = {
        ...
        'AXIS4_SERVO_XL330': 'cots/xl330_cots.stl',
        ...
        'AXIS2_LINK_ADAPTER': 'gearbox/axis2_link_adapter.stl',
        ...
        # Branding
        'DARK_MATTER_LAB_TEXT': 'dark_matter_lab_text.stl',
    }
...
def main():
    ...
    output_path = base_dir / 'output' / 'renders' / 'assembly_xray_4view.png'

    print("="*70)
    print("4-VIEW X-RAY VISUALIZATION - Robot Assembly")
    print("="*70)
    ...
    # Create figure with 2x2 grid for 4 views
    print("\n[4/4] Rendering 4-view x-ray visualization...")
    fig = plt.figure(figsize=(16, 16), facecolor='white')
    fig.suptitle('Robot Assembly - 4-View X-Ray Visualization',
    ...
    views = [
        ...
        (0, 0, 'Front View\n(looking along +X)'),
        (90, 0, 'Top View\n(looking along +Z, down)'),
        (0, -90, 'Right View\n(looking along -Y)'),
    ]

    # Render each view
    for idx, (elev, azim, title) in enumerate(views, 1):
        print(f"      Rendering view {idx}/4: {title.split('(')[0].strip()}")
        ...
        # NOTE: No annotation labels in this version for cleaner visualization
    ...
    # Summary
    print("\n" + "="*70)
    print("4-VIEW X-RAY VISUALIZATION COMPLETE")
    print("="*70)
    print(f"Parts rendered: {part_count}")
    print(f"Parts missing: {missing_count}")
    print(f"Output: {output_path}")
    print("\nViews generated:")
    print("  1. Perspective - Isometric 3/4 view (elev=30°, azim=45°)")
    print("  2. Front       - Looking at YZ plane (along +X)")
    print("  3. Top         - Looking at XY plane from above (along +Z)")
    print("  4. Right       - Looking at XZ plane (along -Y)")
    print("\nKey features:")
    print("  - Transparent x-ray rendering with visible edges")
    print("  - Color-coded by subsystem")
    print("  - Coordinate axes shown (RGB = XYZ)")
    print("  - Clean visualization without annotation labels")
    print("="*70)
