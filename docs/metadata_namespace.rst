...
.. list-table::
   ...
   * - ``schema``
     - string
     - Version identifier, e.g. ``metadata-namespace-v0.1``.
   * - ``entityId``
     - string
     - Stable UUID for the geometry entity (use ``ensure_*_id``).
   * - ``timestamp``
     - string
     - Optional ISO 8601 timestamp of last update.
   * - ``tags``
     - string[]
     - Free-form labels (e.g. ``["prototype", "revision-A"]``).
   * - ``layer``
     - string
     - Logical drawing layer for the entity (``"default"`` if unspecified).
...
.. list-table::
   ...
   * - ``name``
     - string
     - Human-readable descriptor (``"6061 Aluminum"``, ``"PLA"``).
   * - ``standard``
     - string
     - Reference to published spec (``"ASTM B209-14"``).
   * - ``grade``
     - string
     - Alloy/temper or plastic grade (``"6061-T6"``).
   * - ``density_kg_m3``
     - number
     - Density if known.
   * - ``source``
...
.. list-table::
   ...
   * - ``process``
     - string
     - Primary process (``"waterjet"``, ``"SLA"``, ``"3-axis CNC"``).
   * - ``instructions``
     - string
     - Operator notes or setup text.
   * - ``fixtures``
     - string[]
     - Fixture/tooling identifiers.
   * - ``layers``
     - object
     - Optional map relating drawing layers or geometry subsets to process directives.
   * - ``postprocessing``
     - string[]
     - e.g. ``["deburr", "anodise-clear"]``.
...
.. list-table::
   ...
   * - ``source``
     - string
     - ``"prompt"``, ``"script"``, ``"import"`` etc.
   * - ``context``
     - string
     - Prompt text, script path, or design brief summary.
   * - ``tools``
     - string[]
     - Tools/agents used.
   * - ``iterations``
...
.. list-table::
   ...
   * - ``mass``
     - object
     - ``{ "max_kg": 5.0, "target_kg": 4.2 }``.
   * - ``envelope``
     - object
     - Bounding box or volume limits.
   * - ``performance``
     - object[]
     - Domain-specific constraints (e.g. load, stiffness).
   * - ``compliance``
     - string[]
     - Required certifications (``["ASME Y14.5-2018"]``).
...
.. list-table::
   ...
   * - ``plans``
     - string[]
     - IDs referencing ``manifest.validation.plans`` entries executed against this entity.
   * - ``loadcases``
     - object[]
     - Load cases with fields such as ``id``, ``description``, ``loads``, and ``boundaryConditions``.
   * - ``materials``
     - object
     - Overrides mapping entity regions to material records.
   * - ``results``
     - object[]
     - Normalised summaries referencing ``validation/results`` artefacts.
   * - ``safetyFactors``
     - object
     - Calculated safety factors per load case, e.g. ``{"axial-load": 1.8}``.
   * - ``backendHints``
     - object
     - Optional hints for solver adapters.
   * - ``notes``
     - string
     - Free-form context for analysts.
